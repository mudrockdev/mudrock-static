<script lang="ts">
  ...
  let { node, parentPath = "", onfileclick }: { 
    node: TreeNode; 
    parentPath?: string; 
    onfileclick?: (event: { path: string }) => void 
  } = $props();
  ...
  <div class="group mb-1">
    ...
    >
      <div class="flex items-center justify-center w-5 h-5 mr-3">
        <svg 
          class="w-4 h-4 text-slate-500 group-hover:text-slate-300 transition-transform duration-200 {open ? 'rotate-90' : ''}"
          fill="currentColor"
          viewBox="0 0 20 20"
        >
      ...
      <div class="flex items-center">
        <svg class="w-4 h-4 mr-2 text-blue-400 group-hover:text-blue-300" fill="currentColor" viewBox="0 0 20 20">
          <path d="M2 6a2 2 0 012-2h5l2 2h5a2 2 0 012 2v6a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" />
        </svg>
        <span class="font-medium text-slate-300 group-hover:text-slate-100 text-sm">{node.name}</span>
      </div>
    </button>
    {#if open && node.children}
      <div class="ml-6 pl-3 border-l border-slate-700/50 space-y-0.5" role="group" aria-label="Folder contents">
...
{:else if node.type === "file"}
  <div class="group mb-0.5">
    ...
    >
      <div class="flex items-center justify-center w-5 h-5 mr-3">
        <svg class="w-4 h-4 text-slate-500 group-hover:text-slate-300" fill="currentColor" viewBox="0 0 20 20">
